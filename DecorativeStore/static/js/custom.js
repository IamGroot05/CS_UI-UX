$(function () {
            ...
            success: function (data) {
              ...
              if (data.auth) {
                if (data.send) {
                  ...
                  Swal.fire(
                    "Your mail id is shared with our seller.",
                    "",
                    "success"
                  );
                } else {
  ...
  var loadMoreRows = function () {
    let i;
    for (i = 0; i < 5; i++) {
      $(".tablemanager").find("tbody tr").eq(i).addClass("active");
    }
        ...
        if (data.result.data.is_valid) {
          $("#gallery tbody").prepend(
            "<tr><td><a href='" +
              ...
              "'>" +
              data.result.data.image_name +
              "</a></td><td><a href=" +
              "http://127.0.0.1:8000/accounts/photo-delete/" +
              data.result.data.id +
              "><span class='remove-product-image p-1'><img src='https://s.svgbox.net/materialui.svg?ic=delete&fill=13775a' width='26' height='26'></span></a></td></tr>"
          );
        }
      },
    });
        ...
        if (result.isConfirmed) {
          $.ajax({
            ...
            success: function (data) {
              ...
              if (data.status) {
                Swal.fire("Cancelled!", "order item is cancelled.", "success");
              } else {
                Swal.fire(
                  "Failed!",
        ...
        if (result.isConfirmed) {
          $.ajax({
            ...
            success: function (data) {
              ...
              console.log(data.status);
              if (data.status) {
                Swal.fire("Return!", "order item is returned.", "success");
              } else {
                Swal.fire(
                  "Failed!",
                  "Some error occured, Try again.",
  ...
  var loadSubCategory = function () {
    $("#itemCategory").change(function () {
      var id = $(this).val();
      const url =
        "/accounts/dashboard/product/creative/add/get-sub-crt-cat/" + id;
      console.log(url);
      $.ajax({
        url: url,
        dataType: "json",
